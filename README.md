...
ignored. [Every citation, checked against the source](docs/the-evidence.md).

### Watch it happen

One real job, big enough to run for minutes. Two follow-ups typed while it
builds, phrased the way anyone types them.

**Before, stock Claude Code.** The message typed second was started first, and
nothing on screen says so:

![Before: stock Claude Code runs the two typed follow-ups in the wrong order](docs/images/before-stock.gif)

**After, patched.** Same job, same two messages, sent after the same verified
tool milestones. Each waits, then runs in the order typed:

![After: with claude-queue both follow-ups wait and run in the order they were typed](docs/images/after-patched.gif)
...
**Verified on Claude Code 2.1.220** (last checked 2026-07-27). On a release
...
No `curl | bash` here: you clone it, read it, run it.
...
![Each queued message is labelled waits or jumps in](docs/images/feat-labels.gif)
...
![A message with no marker waits for the turn to end](docs/images/feat-queue.gif)

![An s message jumps into the running turn](docs/images/feat-steer.gif)
...
![Up highlights, enter brings back only that one](docs/images/feat-edit.gif)
...
![Shift with an arrow moves the highlighted message](docs/images/feat-reorder.gif)
...
![Delete removes the highlighted message](docs/images/feat-delete.gif)
...
![Four marked lines become three separate jobs](docs/images/feat-split.gif)
...
![With the default flipped, no marker jumps in and q still waits](docs/images/feat-default-steer.gif)

![With drain=all, both waiting messages arrive in one turn](docs/images/feat-drain-all.gif)
...
operation. Full detail, edit by edit: [docs/behaviour.md](docs/behaviour.md).
